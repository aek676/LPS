...
class ViewModel: ObservableObject {
    ...
    func cargarDatos() {
        ...
        let fetchPersonas = NSFetchRequest<PersonaEntity>(
            entityName: "PersonaEntity")
        let fetchMascotas = NSFetchRequest<MascotaEntity>(
            entityName: "MascotaEntity")
        do {
            self.personasArray = try gestorCoreData.contexto.fetch(
                fetchPersonas
            ).sorted { $0.nombre! < $1.nombre! }
            self.mascotasArray = try gestorCoreData.contexto.fetch(
                fetchMascotas
            ).sorted { $0.nombre! < $1.nombre! }
        } catch let error {
    ...
    func addMascota(
        persona: PersonaEntity, nombre: String, edad: Int16, raza: String
    ) {
